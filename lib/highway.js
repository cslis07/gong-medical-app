...
import { TOLLGATES } from "../data/ex-tollgates.js";   // 영업소 목록(빌드타임 생성, 소용량)
...
// 실시간 문자정보(돌발·사고·공사·정체 VMS). altitude 필드가 경도다.
async function smsInfo() {
  const d = await exGet("burstInfo/realTimeSms", { numOfRows: "100", pageNo: "1", pagingYn: "Y", sortType: "desc" });
  if (d._blocked) return { blocked: true, rows: [] };
  const list = d.realTimeSMSList || [];
  const rows = list
    .filter((r) => String(r.accTypeCode) !== "15")   // '이벤트/홍보'(상황실 문의 등) 노이즈 제외
    .map((r) => ({
      type: r.accType || "", typeCode: String(r.accTypeCode || ""),
      route: (r.roadNM || "").trim(), routeNo: (r.nosunNM || "").trim(),
      point: (r.accPointNM || "").trim(), dir: (r.startEndTypeCode || "").trim(),
      text: (r.smsText || "").trim(), process: (r.accProcessNM || "").trim(),
      lateLength: r.lateLength != null && r.lateLength !== "" ? Number(r.lateLength) : null,
      date: r.accDate || "", time: r.accHour || "",
      lat: numOrNull(r.latitude), lon: numOrNull(r.altitude),   // altitude = 경도
      lanesClosed: [r.laneYn1, r.laneYn2, r.laneYn3, r.laneYn4, r.laneYn5, r.laneYn6].filter((v) => v === "Y").length,
      shoulder: r.shldroadYn === "Y",
    }));
  // 사고·공사·통제 우선, 그 다음 정체길이 큰 순
  const pri = (t) => (/사고|공사|통제|재난|낙하/.test(t) ? 2 : /정체|서행/.test(t) ? 1 : 0);
  rows.sort((a, b) => pri(b.type) - pri(a.type) || (b.lateLength || 0) - (a.lateLength || 0));
  return { rows: rows.slice(0, 60) };
}
...
export default async function handler(req, res) {
  try {
    ...
    if (op === "sms") {
      const out = await smsInfo();
      if (out.blocked) return res.status(200).json({ ok: false, message: "고속도로 포털 접근이 일시 차단되었습니다." });
      return res.status(200).json({ ok: true, ...out });
    }
    if (op === "tollgates") {   // 영업소 목록(드롭다운용)
      return res.status(200).json({ ok: true, list: TOLLGATES });
    }
    if (op === "traveltime") {  // 구간 실시간 소요시간
      const start = String(req.query.start || "").trim(), end = String(req.query.end || "").trim();
      if (!start || !end) return res.status(400).json({ error: "출발/도착 영업소코드가 필요합니다." });
      if (start === end) return res.status(200).json({ ok: true, found: false, message: "출발과 도착이 같습니다." });
      const d = await exGet("trtm/realUnitTrtm", { iStartUnitCode: start, iEndUnitCode: end, numOfRows: "300", pageNo: "1" });
      if (d._blocked) return res.status(200).json({ ok: false, message: "고속도로 포털 접근이 일시 차단되었습니다." });
      const list = d.realUnitTrtmVO || [];
      // 1종(승용) + HH:MM 시각 행 중 최신을 '현재'로
      const car1 = list.filter((r) => String(r.tcsCarTypeCode) === "1" && /^\d{1,2}:\d{2}$/.test(String(r.stdTime || "").trim()));
      car1.sort((a, b) => String(b.stdTime).trim().localeCompare(String(a.stdTime).trim()));
      const cur = car1[0];
      if (!cur) return res.status(200).json({ ok: true, found: false, message: "해당 구간의 실시간 소요시간이 없습니다. (직접 연결 구간만 제공)" });
      return res.status(200).json({
        ok: true, found: true,
        startNm: (cur.startUnitNm || "").trim(), endNm: (cur.endUnitNm || "").trim(),
        timeAvg: Math.round(Number(cur.timeAvg)), timeMax: Math.round(Number(cur.timeMax)), timeMin: Math.round(Number(cur.timeMin)),
        stdTime: String(cur.stdTime || "").trim(), stdDate: cur.stdDate || "",
      });
    }
    // op=rest
    ...
    return res.status(500).json({ error: errorMessage(err, "고속도로") });
  }
}
