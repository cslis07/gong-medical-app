...
import NATIONWIDE, { generatedAt as nationwideAt } from "../data/parking-nationwide.js";
...
function nationwideRows(lat, lon) {
  const out = [];
  ...
  try {
    if (String(req.query.diag) === "1") {
      return res.status(200).json({
        ok: true,
        nationwide: { rows: NATIONWIDE.length, generatedAt: nationwideAt },
    ...
    const [seoul, kotsa] = await Promise.all([
    ...
    const merged = dedupe([...seoul.rows, ...kotsa.rows, ...nationwideRows(lat, lon)]);
