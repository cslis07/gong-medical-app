  ...
  try {
    ...
    if (kind === "stats") {
      ...
      if (/^\d{6}$/.test(ymRaw)) {
        ...
        // 최대 31일치를 한 번에 던지면 서울 API에 버스트가 걸리고, 이 엔드포인트가
        // 요청 1건당 상위 호출 31회짜리 증폭기가 된다. 동시성을 6으로 묶는다.
        const { results } = await pool(dates, 6, fetchDay);
        const agg = new Map(); // 호선|역명 → 합산
        for (const day of results) {
          if (!day) continue;
          for (const x of dayRows(day)) {
    ...
    if (kind === "shortestPath") {
      ...
      // j._raw(상위 응답 원문)는 사용자에게 반사하지 않는다.
      if (!ok) {
        if (j._raw) console.error("[지하철] 최단경로 응답이 JSON 아님:", j._raw.slice(0, 200));
        return res.status(200).json({ code: j.header?.resultCode ?? "ERR", message: j.header?.resultMsg ?? "조회 실패", total: 0, rows: [] });
      }
    ...
    return res.status(500).json({ error: errorMessage(err, "지하철") });
  }
}
