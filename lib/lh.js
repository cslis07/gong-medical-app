...
function buildQs(req, page) {
  ...
  // 선택 필터: 지역코드(CNP_CD), 공고상태(PAN_SS), 공고명(PAN_NM), 유형(UPP_AIS_TP_CD), 게시기간
  for (const [q, p] of [["region", "CNP_CD"], ["status", "PAN_SS"], ["name", "PAN_NM"], ["type", "UPP_AIS_TP_CD"], ["from", "PAN_ST_DT"], ["to", "PAN_ED_DT"]]) {
...
async function callPage(req, page) {
  ...
  let j; try { j = JSON.parse(t); } catch { throw new Error(`LH 응답 파싱 실패: ${t.slice(0, 120)}`); }
