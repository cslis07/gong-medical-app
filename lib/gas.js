...
export default async function handler(req, res) {
  try {
    ...
    // 최근 7일 전국 평균유가 추이 (avgRecentPrice) — 유종별 스파크라인용
    if (String(req.query.op) === "recent") {
      const prodcd = PRODS.has(String(req.query.prodcd)) ? String(req.query.prodcd) : "B027";
      const j = await getJson(`${OPINET}/avgRecentPrice.do?out=json&code=${encodeURIComponent(KEY)}&certkey=${encodeURIComponent(KEY)}`);
      let oil = j?.RESULT?.OIL || [];
      if (!Array.isArray(oil)) oil = oil ? [oil] : [];
      const series = oil.filter((o) => o.PRODCD === prodcd)
        .map((o) => ({ date: String(o.DATE), price: Number(o.PRICE) || null }))
        .filter((p) => p.price != null)
        .sort((a, b) => a.date.localeCompare(b.date));
      return res.status(200).json({ ok: true, prodcd, series });
    }
    ...
    const rows = top.map((o, i) => {
      const dt = details[i] || {};
      // 지도 표시용 WGS84 좌표 (aroundAll은 KATEC GIS 좌표를 준다)
      let glat = null, glon = null;
      const gx = Number(o.GIS_X_COOR), gy = Number(o.GIS_Y_COOR);
      if (Number.isFinite(gx) && Number.isFinite(gy) && gx !== 0 && gy !== 0) {
        try { const [lo, la] = proj4(KATEC, "WGS84", [gx, gy]); glat = la; glon = lo; } catch { /* 좌표 변환 실패 시 무시 */ }
      }
      return {
        id: o.UNI_ID,
        name: o.OS_NM,
