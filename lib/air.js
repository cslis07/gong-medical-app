...
export default async function handler(req, res) {
  try {
    ...
    // 시도별 실시간 측정정보 + 대기질 예보(PM10·PM2.5) 병렬
    const now = new Date(Date.now() + 9 * 3600e3);
    const today = `${now.getUTCFullYear()}-${String(now.getUTCMonth() + 1).padStart(2, "0")}-${String(now.getUTCDate()).padStart(2, "0")}`;
    const fcUrl = (code) => `${BASE}/getMinuDustFrcstDspth?serviceKey=${key}&returnType=json&searchDate=${today}&InformCode=${code}&numOfRows=10&pageNo=1`;
    const [rt, fc10, fc25] = await Promise.all([
      getJson(rtUrl(sido)),
      getJson(fcUrl("PM10")),
      getJson(fcUrl("PM25")),
    ]);
    ...
    // 예보문 — API가 InformCode 필터를 무시하고 PM10·PM2.5·O3를 섞어 주므로
    // 응답에서 해당 코드만 골라 가장 최근 발표(informData 최댓값)를 쓴다.
    const mkFc = (j, code) => {
      let a = j?.response?.body?.items ?? [];
      if (!Array.isArray(a)) a = a ? [a] : [];
      const matched = a.filter((x) => x.informCode === code).sort((p, q) => String(q.informData || "").localeCompare(String(p.informData || "")));
      const it = matched[0];
      if (!it) return null;
      ...
      return { overall: it.informOverall, cause: it.informCause, sidoGrade, date: it.informData };
    };
    const forecast = mkFc(fc10, "PM10");
    const forecastPm25 = mkFc(fc25, "PM25");

    return res.status(200).json({ ok: true, sido, summary, forecast, forecastPm25, stations: stations.sort((a, b) => (b.pm10 || 0) - (a.pm10 || 0)) });
  } catch (err) {
