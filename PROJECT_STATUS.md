...
- **시크릿 3종 등록 완료(2026-08-13, 사용자)**: `DATA_API_KEY`·`EX_API_KEY`·`VERCEL_TOKEN`. 이제 월 1회 갱신+배포 완전 자동. (CLI 403은 histobio 계정 탓 → 웹 UI로 해결.)
- **자동 갱신 대상은 주차장·야간진료뿐**: 테스트 실행에서 **고속도로 빌드가 CI 10분+ 소모**하는 걸 발견해 월간에서 뺐다(영업소는 거의 불변, `realUnitTrtm` 수백 페이지 스캔). `build:tollgates`는 수동 전용 + 안전장치(잘리면 566곳 보존). 키 없으면 조용히 건너뜀(실패 메일 없음).
- **다음 채팅이 가장 먼저 할 한 가지**: 특별히 없음(모든 작업 배포 완료, 자동화 검증 중).
