...
- **`DATA_API_KEY` GitHub 시크릿(선택)**: 등록하면 월 1회 스냅샷 자동 갱신 시작. **없어도 워크플로가 조용히 건너뛴다**(실패·알림 메일 없음, 2026-08-12에 그렇게 고침). 등록은 사용자 몫(키를 다루는 일이라 Claude가 대신 안 함). 웹 UI가 가장 확실(계정 꼬임 회피). 안 넣으면 스냅샷은 세션 때 손으로 `npm run build:*` 후 배포.
- **다음 채팅이 가장 먼저 할 한 가지**: 특별히 없음(모든 작업 배포 완료).
